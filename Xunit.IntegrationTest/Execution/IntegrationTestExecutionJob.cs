...
namespace Xunit.IntegrationTest.Execution
{
	...
	{
		public static ExecutionJob Create(IntegrationTestCase testCase, IMessageBus messageBus, CancellationTokenSource cancellationTokenSource)
		{
			if (testCase is ErrorIntegrationTestCase errorTestCase)
				return ExecutionJob.Create(testCase.Method.ToRuntimeMethod(), errorTestCase.ErrorMessage, arguments => ExecuteTest(testCase, messageBus, arguments, cancellationTokenSource), abortMessage => AbortTest(testCase, messageBus, abortMessage));

			return ExecutionJob.Create(testCase.Method.ToRuntimeMethod(), arguments => ExecuteTest(testCase, messageBus, arguments, cancellationTokenSource), abortMessage => AbortTest(testCase, messageBus, abortMessage));
		}
		...
		private static async Task ExecuteTest(IntegrationTestCase testCase, IMessageBus messageBus, object[] arguments, CancellationTokenSource cancellationTokenSource)
		{
			SendStartMessages(testCase, messageBus);

			await testCase.Execute(Array.Empty<object>(), arguments, messageBus, new ExceptionAggregator(), cancellationTokenSource);

			SendStopMessages(testCase, messageBus);
		}

		private static void FailTest(IntegrationTestCase testCase, IMessageBus messageBus, string errorMessage)
		{
